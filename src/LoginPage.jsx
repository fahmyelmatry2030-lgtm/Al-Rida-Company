...
export default function LoginPage({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Default admin always works instantly - no network needed
    if (username.trim() === 'admin' && password === 'admin') {
      onLogin({ id: 'admin', username: 'admin', role: 'admin', name: 'المدير' });
      return;
    }

    // For other users, query Firestore with 8-second timeout
    try {
      const timeout = new Promise((_, reject) =>
        setTimeout(() => reject(new Error('انتهت مهلة الاتصال. تحقق من الإنترنت أو استخدم admin/admin')), 8000)
      );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" dir="rtl"
      style={{ fontFamily: "'Cairo', sans-serif", background: 'linear-gradient(135deg, #1e1b4b 0%, #312e81 30%, #4338ca 60%, #6366f1 100%)' }}>
      ...
      <div className="absolute top-20 right-20 w-3 h-3 bg-white/20 rounded-full animate-pulse pointer-events-none"></div>
      <div className="absolute bottom-32 left-32 w-2 h-2 bg-white/15 rounded-full animate-pulse pointer-events-none" style={{ animationDelay: '1s' }}></div>

      <div className="w-full max-w-md relative z-10">
        {/* Logo */}
        <div className="text-center mb-8">
          <div className="w-20 h-20 mx-auto rounded-3xl bg-white/10 backdrop-blur-xl border border-white/20 flex items-center justify-center mb-5 shadow-2xl shadow-indigo-900/50"
        ...
        <div className="bg-white/10 backdrop-blur-2xl rounded-3xl p-8 border border-white/20 shadow-2xl shadow-black/20">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={username} onChange={e => setUsername(e.target.value)}
                  ...
                  className="w-full pr-12 pl-4 py-3.5 bg-white/10 border border-white/10 rounded-2xl text-white placeholder-white/30 focus:ring-2 focus:ring-indigo-400 focus:border-transparent outline-none transition-all text-sm"
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3.5 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white rounded-2xl font-bold text-sm flex items-center justify-center gap-2 shadow-xl shadow-indigo-500/30 transition-all active:scale-[0.98] disabled:opacity-60">
              {loading
