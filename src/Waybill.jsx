...
export default function Waybill({ order, orders, onClose }) {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html dir="rtl">
      <head>
        <title>طباعة بوالص الشحن</title>
        <style>
          @import url('https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800;900&display=swap');
          * { margin: 0; padding: 0; box-sizing: border-box; }
          body { font-family: 'Cairo', sans-serif; padding: 10mm; background: #fff; color: #1e293b; }
          .waybill { border: 2px solid #1e293b; border-radius: 8px; overflow: hidden; max-width: 380px; margin: 0 auto 25px; background: #fff; page-break-after: always; }
          .waybill:last-child { page-break-after: avoid; margin-bottom: 0; }
          @media print {
            body { padding: 0; }
            .waybill { margin: 0 auto; page-break-after: always; border: 2px solid #1e293b !important; }
            .waybill:last-child { page-break-after: avoid; }
          }
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <h2 className="text-lg font-black text-slate-800">
            معاينة بوالص الشحن ({ordersList.length})
          </h2>
          <div className="flex items-center gap-2">
            <button onClick={handlePrint} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl font-bold text-sm transition-colors shadow-md shadow-indigo-600/10">
        ...
        <div className="p-6 overflow-y-auto bg-slate-100 flex-1">
          <div ref={printRef} className="space-y-6">
            {ordersList.map((o) => (
              <div key={o.id} className="waybill" style={{border: '2px solid #1e293b', borderRadius: '8px', overflow: 'hidden', maxWidth: '380px', margin: '0 auto 20px', background: '#fff', pageBreakAfter: 'always'}}>
                ...
                <div style={{textAlign: 'center', padding: '12px 0 8px', borderBottom: '1px dashed #cbd5e1'}}>
                  <Barcode value={o.code || o.id || '0000'} width={2} height={50} fontSize={12} margin={0} displayValue={true} />
                </div>
                ...
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px dashed #cbd5e1'}}>
                  <div style={{padding: '8px 12px', borderLeft: '1px dashed #cbd5e1'}}>
                    ...
                    <div style={{fontSize: '13px', fontWeight: 700, color: '#1e293b'}}>{o.date}</div>
                  </div>
                  <div style={{padding: '8px 12px'}}>
                    <div style={{fontSize: '9px', color: '#94a3b8', fontWeight: 600, marginBottom: '2px'}}>الراسل / التاجر</div>
                    <div style={{fontSize: '13px', fontWeight: 700, color: '#1e293b'}}>{o.company || o.sender || '—'}</div>
                  </div>
                </div>
                ...
                <div style={{padding: '8px 12px', borderBottom: '1px dashed #cbd5e1'}}>
                  ...
                  <div style={{fontSize: '15px', fontWeight: 800, color: '#1e293b'}}>{o.customerName || '—'}</div>
                </div>

                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px dashed #cbd5e1'}}>
                  <div style={{padding: '8px 12px', borderLeft: '1px dashed #cbd5e1'}}>
                    <div style={{fontSize: '9px', color: '#94a3b8', fontWeight: 600, marginBottom: '2px'}}>📞 رقم الهاتف</div>
                    <div style={{fontSize: '13px', fontWeight: 700, color: '#1e293b'}} dir="ltr">{o.phone || '—'}</div>
                  </div>
                  <div style={{padding: '8px 12px'}}>
                    <div style={{fontSize: '9px', color: '#94a3b8', fontWeight: 600, marginBottom: '2px'}}>📍 المنطقة</div>
                    <div style={{fontSize: '13px', fontWeight: 700, color: '#1e293b'}}>{o.center || '—'}</div>
                  </div>
                </div>
                ...
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px dashed #cbd5e1'}}>
                  <div style={{padding: '8px 12px', borderLeft: '1px dashed #cbd5e1'}}>
                    ...
                    <div style={{fontSize: '13px', fontWeight: 700, color: '#1e293b'}}>{o.count || 1}</div>
                  </div>
                  <div style={{padding: '8px 12px'}}>
                    <div style={{fontSize: '9px', color: '#94a3b8', fontWeight: 600, marginBottom: '2px'}}>🚚 المندوب</div>
                    <div style={{fontSize: '13px', fontWeight: 700, color: '#1e293b'}}>{o.agent || '—'}</div>
                  </div>
                </div>
                ...
                <div style={{background: '#f8fafc', padding: '14px 16px', textAlign: 'center', borderBottom: '1px dashed #cbd5e1'}}>
                  ...
                  <div style={{fontSize: '28px', fontWeight: 900, color: '#1e293b'}}>
                    {Number(o.total || 0).toLocaleString()} <span style={{fontSize: '14px', fontWeight: 600, color: '#94a3b8'}}>ج.م</span>
                  </div>
                </div>
