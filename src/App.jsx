...
function App() {
  ...
  const deleteOrder = async (id) => {
    if (currentUser?.role !== 'admin') {
      alert('خطأ: هذه الصلاحية مخصصة لمدير النظام فقط.');
      return;
    }
    const order = orders.find(o => o.id === id);
    if (order?.settled) return alert('لا يمكن حذف طلب تم تقفيله حسابياً.');
  ...
  const summaryStats = useMemo(() => {
    return filteredOrders.reduce((acc, order) => {
      ...
      acc.totalNet += calculateNet(order.collected, sFee, order.status);
  ...
  const exportOrdersToExcel = () => {
    const dataToExport = filteredOrders.map((order, i) => {
      ...
      return {
        ...
        'الصافي': calculateNet(order.collected, sFee, order.status),
                        ...
                        <tr key={order.id} onClick={() => activeTab !== 'archive' && !isAgent && openEditModal(order)} className={`border-b border-slate-100/80 ${activeTab !== 'archive' && !isAgent ? 'cursor-pointer' : ''} transition-all duration-150 ${isCanceled ? 'opacity-50' : ''} ${isSelected ? 'bg-indigo-50/50' : order.settled ? 'bg-emerald-50/30' : 'hover:bg-indigo-50/40'}`}>
                          ...
                          <td className="px-3 py-2 text-center font-black text-indigo-755 text-[15px]">
                            {(() => {
                              ...
                              return calculateNet(order.collected, sFee, order.status).toLocaleString();
                            })()}
                          </td>
                          <td className="px-3 py-2 text-center" onClick={e => e.stopPropagation()}>
                            <input 
